  ...
  <div style="height:100%">
    ...
    <i-Col span="12"
      ...
      <div style="width:100%;justify-content:space-between;align-items:center; ">
        ...
        <!-- 报表列表授权 -->
        <Select v-model="statementSearchVal"
                filterable
                clearable
                id="selectlIId"
                remote
                placeholder="请输入名称查询"
                @on-change="statementSearchPosition"
                :remote-method="statementSearchMethod"
                style="width: 200px;margin-left:10px">
          <Option v-for="(option, index) in statementOptions"
                  :value="option.label"
      ...
      <div id="tableSelectId"
        ...
        <span style="border:1px solid gery">
          <Tree class="room-tree-contentbbsq"
                ref="bbsqRoom"
                :data="baobAutList"
                @on-check-change="treeBabsqCance"
                show-checkbox></Tree>
          <Spin size="large"
                fix
                v-if="rightbbsqLoading"></Spin>
        </span>
      </div>
    </i-Col>
    <!-- 新增/修改 -->
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rightbbsqLoading: false,
      baobAutList: [],           //获取自定义报表树节点
      baobAutListClon: [],     //用于深拷贝
      statementSearchVal: '',   //报表授权的搜索下拉框回显数据
      statementOptions: [],//报表授权的搜索下拉框列的数据
      baobAutListFB: [],   //报表授权的右侧数据
  ...
  methods: {
    init () {// 初始化
      ...
      this.customReportDictAllList() //获取报表授权全部数据
    },
    addOrEditor () {   //新增事件 
      this.changeModel = true;
      this.addOrUp = 1;
    ...
    customReportDictAllList () {    // 获取全部自定义报表树
      this.rightbbsqLoading = true
      this.$store.dispatch('customReportDictList', { hospitalId: this.$store.state.userInfoObj.hospitalId }).then(res => {
        if (res.data.code !== 200) {
          util.showMsg(this, { diy: res.data.message });
        } else {
          let resList = util.getTree(res.data.data, 'customReportId', 'reportName', 'parentId', '全部报表')
          this.baobAutListFB = res.data.data
          this.baobAutList = resList[0].children
          this.baobAutListClon = resList[0].children
        }
        this.rightbbsqLoading = false
      })
    },
    treeBabsqCance (val) {    // 报表授权新增取消
      if (util.isEmpty(this.groupIds)) {
        util.showMsg(this, { warning: '请选择资源组或用户' })
        return
      }
      if (this.babsqNum == false) {
        this.babsqNum = true
        return
      }
      this.rightbbsqLoading = true
      ...
      let arry = []
      if (selectList.length > this.bbsqAllCheck.length) {
        //新增
        selectList.map(item => {
          const index = util.indexOfObject(item.id, this.bbsqAllCheck, 'resourceId')
          if (index == -1) {
            const obj = {}
            obj["hospitalId"] = this.$store.state.userInfoObj.hospitalId
        ...
        this.$store.dispatch('batchAddftzs', param).then(res => {
          ...
          if (res.data.code !== 200) {
            ...
            util.showMsg(this, { succ: '新增成功' });
            this.getbbsqCheckData(this.groupIds)
          }
        })
      } else {
        //单个删除
        let arr = []
        this.bbsqAllCheck.map(item => {
          if (val.length > 0) {
    ...
    statementSearchMethod (query) {    //报表远程搜素
      if (query !== '') {
        setTimeout(() => {
          const list = this.baobAutListFB.map(item => {
            return {
          ...
          this.statementOptions = util.findKeys(query, list, ['pyCodeLower', 'pyCodeUpper', 'label', 'wbCode'])
        }, 10);
      } else {
        this.statementOptions = [];
      }
    },
    statementSearchPosition () {    //搜索报表定位
      if (!util.isEmpty(this.statementSearchVal)) {
        let id = util.findOfObject(this.statementSearchVal, this.baobAutListFB, 'reportName', 'customReportId', 'wbCode', 'pyCode');
        if (id) {
          let treeItems = {};
          this.baobAutList.map(val => {
            if (util.treeMap(val, id)) {
              treeItems = util.treeMap(val, id);
            }
          })
          if (!treeItems) {
            util.showMsg(this, { warning: `没有${this.statementSearchVal}` });
            return;
          }
          if (treeItems.nodeKey != undefined) {
            this.$refs['bbsqRoom'].handleSelect(treeItems.nodeKey);
            this.$nextTick().then(
              () => {
                util.isEmpty(document.querySelector('.room-tree-contentbbsq span.ivu-tree-title.ivu-tree-title-selected')) ? '' : document.querySelector('.room-tree-contentbbsq span.ivu-tree-title.ivu-tree-title-selected').scrollIntoView({ behavior: "instant", block: "end", inline: "nearest" });
              }
            )
          }
        }
    ...
    getbbsqCheckData (val) {    // 报表授权回显
      this.baobAutList = JSON.parse(JSON.stringify(this.baobAutListClon));
      this.rightbbsqLoading = true
      let nowData = {
        groupId: val.groupId,
        hospitalId: this.$store.state.userInfoObj.hospitalId,
        ...
        if (res.data.code !== 200) {
          ...
          this.baobAutList.map(ite => {
            ...
            res.data.data.list.map(item => {
              ...
              if (index != undefined) {
                hasCheck.push(index)
                index.children.length == 0 ? this.bbsqCheck.push(item) : ''
                this.bbsqAllCheck.push(item)
              }
            })
          })
          hasCheck.map((vals, i) => {
            ...
            if (vals.children.length == 0) {
              this.babsqNum = false;
              this.$refs['bbsqRoom'].handleCheck(varData)
            }
            // 排序，选中在前
            this.listSort(this.baobAutList, vals)
          })
        }
        this.rightbbsqLoading = false
      })
