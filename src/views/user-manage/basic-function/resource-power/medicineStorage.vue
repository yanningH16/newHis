  ...
  <div style="height:100%">
    ...
    <i-Col span="12"
      ...
      <div style="width:100%;justify-content:space-between;align-items:center; ">
        ...
        <!-- 药库授权 -->
        <Input v-model="warekGroup"
               style="width: 200px;margin-left:10px"
               placeholder="请输入名称查询"
               @on-change="warekGroupTabel"
               @keyup.native.enter="warekGroupTabel" />
      </div>
      <div id="tableSelectId"
           style="width:100%;overflow-y:auto;">
        <Table :loading="rightyfLoading"
               border
               ref="selectionRightYf"
               :height="tableHightRt"
               stripe
               :columns="columnsyk"
               :data="datayk"
               @on-row-click="yftaleClick"
               @on-selection-change="yaokCance"
               style="margin-top:10px"></Table>
      </div>
    </i-Col>
    <!-- 新增/修改 -->
...
export default {
  ...
  data () {
    ...
    return {
      ...
      datayk: [],     //药库授权的用户左侧表格数据
      dataykEB: [],
      warekGroup: '',       //药库授权的input搜索框回显数据
      rightyfLoading: false,
      ykChecked: []
    }
  },
  mounted () {
    this.tableHightLt = (window.innerHeight - this.$refs.selectionRightYf.$el.offsetTop - 300) / 2;       //资源组左侧两个表格的自定义高度
    this.tableHightUser = window.innerHeight - this.$refs.selectionRightYf.$el.offsetTop - 170;           //用户表格的自定义高度 
    this.tableHightRt = window.innerHeight - this.$refs.selectionRightYf.$el.offsetTop - 170           //右侧表格的自定义高度 
  },
  methods: {
    init () {// 初始化
      this.leftBottomTable()    //获取左下表格
      this.getLeftTable()//获取左上表格
      this.getyaokkuTableAjax() //获取药库全部数据
    },
    addOrEditor () {   //新增事件 
      this.changeModel = true;
      this.addOrUp = 1;
    ...
    tableRowclick (val) {      //点击左侧列表数据获取右侧数据
      this.groupIds = val;
      this.warekGroupTabel(this.warekGroup) //药库权限
    ...
    // 获取药库数据
    getyaokkuTableAjax () {
      this.rightyfLoading = true
      let nowData = {
        enable: 1, //1为启用的药房 空是全部  0是未启用
        flag: 1,  // 1-药品仓库,2-药房,3-窗口
        itemType: 1,
        searchPharm: this.warekGroup
      }
      this.$store.dispatch('getStoragePhamDictTotle', nowData).then(res => {
        if (res.data.code === 200) {
          this.datayk = res.data.data.list;
          this.dataykEB = res.data.data.list;
        } else {
          util.showMsg(this, { diy: res.data.message });
        }
        this.rightyfLoading = false
      })
    },
    // 药库新增取消
    yaokCance (val) {
      ...
      this.rightyfLoading = true
      let arrys = []
      if (val.length > this.ykChecked.length) {
        //新增
        if (val.length > 0) {
          val.map(item => {
            const index = util.indexOfObject(item.deptId, this.ykChecked, 'deptId')
            if (index == -1) {
              const obj = {}
              obj["hospitalId"] = this.$store.state.userInfoObj.hospitalId
          ...
          this.$store.dispatch('batchAddftzs', param).then(res => {
            ...
            if (res.data.code !== 200) {
              ...
              util.showMsg(this, { succ: '新增成功' })
              this.warekGroupTabel(this.warekGroup)
            }
          })
        }
      } else {
    ...
    //点击库房表格
    yftaleClick (val, index) {
      this.$refs.selectionRightYf.toggleSelect(index);
    },
    //搜索药库
    warekGroupTabel () {
      //input搜索
      this.getyaokkuTableAjax();
      setTimeout(() => {
        !util.isEmpty(this.groupIds)
          ? this.getyaokCheckData(this.groupIds)
          : ""; // 库房权限
      }, 50);
    },
    // 药库回显
    getyaokCheckData (val) {
      this.datayk = JSON.parse(JSON.stringify(this.dataykEB));
      this.rightyfLoading = true
      let nowData = {
        groupId: val.groupId,
        hospitalId: this.$store.state.userInfoObj.hospitalId,
      ...
      this.$store.dispatch('getGroupResources', nowData).then(res => {
        if (res.data.code !== 200) {
          ...
          this.ykChecked = []
          this.datayk.forEach(element => {
            element._checked = false
          })
          res.data.data.list.map(item => {
            const index = util.indexOfObject(item.resourceId, this.datayk, 'deptId')
            if (index != -1) {
              this.ykChecked.push(this.datayk[index])
              this.datayk[index]._checked = true
              this.datayk.unshift(this.datayk[index]);
              this.datayk.splice(index + 1, 1);
            }
          })
        }
        this.rightyfLoading = false
